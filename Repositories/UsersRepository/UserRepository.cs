...
namespace Web_Social_network_BE.Repositories.UserRepository
{
    public class UserRepository : IUserRepository
    {
        ...
        public UserRepository(SocialNetworkN01Ver3Context context)
        {
            _context = context;
        }
        ...
        public async Task ChangeAvatar(string userId, string avatar)
        {
            try
            {
                ...
                if (user == null)
                {
                    throw new ArgumentException("Error while find user to update");
                }
            ...
            {
                throw new Exception("Error while change avatar ", ex);
            }
        }

        public async Task ChangeCoverImage(string userId, string coverImage)
        {
            try
            {
                var user = await _context.Users.FirstOrDefaultAsync(x => x.UserId == userId);
                ...
                user.CoverImage = "/images/" + coverImage;
                _context.Users.Update(user);
                await _context.SaveChangesAsync().ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                throw new Exception("Error while cchange image cover");
            }
        }
    }
}
