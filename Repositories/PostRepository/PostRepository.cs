...
namespace Web_Social_network_BE.Repositories.PostRepository
{
    public class PostRepository : IPostRepository
    {
        ...
        public PostRepository(SocialNetworkN01Ver3Context context)
        {
            _context = context;
        }
		...
		public async Task<IEnumerable<Post>> GetAllPostForHomeAsync(string userId)
		{
			var postForHome =await  (from relation in _context.Relations
                                   ...
                                   where post.AccessModifier == "PUBLIC"
			...
			return postForHome.Where(x => !_context.Newfeeds.Any(nf => nf.PostId == x.PostId && nf.Type == "HIDDEN" && nf.UserId == userId));
		}
