...
namespace Web_Social_network_BE.Controller
{
    ...
    public class UserController : ControllerBase
    {
        ...
        [HttpPut("changeAvatar/{avatar}")]
        public async Task<IActionResult> ChangeAvt(string avatar)
        {
            var userId = _session.GetString("UserId");
            await _userRepository.ChangeAvatar(userId, avatar);
            return Ok();
        }
		[HttpPut("changeCoverImage/{coverImage}")]
		public async Task<IActionResult> ChangeImg (string coverImage)
		{
			var userId = _session.GetString("UserId");
			await _userRepository.ChangeCoverImage(userId, coverImage);
			return Ok();
		}
	}
}